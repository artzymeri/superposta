...
const ShtoMbledhesContent = () => {
    ...
    const [presenter, setPresenter] = useState(container.get(TYPES.AdminShtoMbledhesPresenter))
    ...
    return (
        <div className="shto-mbledhes-page-container">
            <div style={{display: 'flex', gap: '15px', flexDirection: 'column'}}>
                <h2>Shto Mbledhes</h2>
                <Button className="kthehu-button" style={{alignSelf: 'start'}} variant="outlined"
                        startIcon={<ArrowBack/>} onClick={() => {
                    router.push('/mbledhesit')
                }}>
                    Mbledhesët
                </Button>
            </div>
            <div className="shto-mbledhes-page-content">
                <div className="shto-mbledhes-page-form-container">
                    ...
                    <FormControl fullWidth variant="filled">
                        <NativeSelect
                            defaultValue=" "
                            placeholder="Vetura e Mbledhesit"
                            sx={{height: '100%', background: 'rgba(0, 0, 0, 0.06)', padding: '0px 0px 0px 10px'}}
                            variant="filled"
                            onChange={(e) => {
                    ...
                    <Button style={{gridColumn: 'span 2'}} size="large" variant="contained" onClick={() => {
                        presenter.addCollector()
                    }}>Shto Mbledhes</Button>
                </div>
            </div>
        </div>
    )
